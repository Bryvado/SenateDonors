...
The map opens on the U.S. overview. **Map view** switches among areas within selected states, U.S. states, and nationwide ZIP (ZCTA), county, congressional district, metro, and county subdivision views. The compact state selector beside the left map controls jumps directly to any state. Switching area types in the same scope keeps the current map position. Nationwide ZCTAs and county subdivisions load boundaries by visible state after zoom level 6; while zoomed out they show state-level shading. The top places list in those views covers loaded states. The location path at the bottom left steps back from an area to the selected states or U.S. overview. Click a state to show it alone, or Shift-click to add it; click an area to focus it. Returning to a selected-state view restores the last selection, or asks you to choose a state if none was selected. Choose a period, two candidates, and a color measure from the masthead. The USGS basemap can be switched off.
